...
int resolve_path(struct client_attr *attr, char *path)
{
	...
	if ((attr->req.code == REQ_CD || attr->req.code == REQ_MKDIR ||
			attr->req.code ==REQ_RM || attr->req.code == REQ_DOWNLOAD) 
			&& (attr->req.len == 0)) {
		return -1;
		
	} else if (attr->req.len == 0)	{ /* No arguments */
		strcpy(path, attr->cwd);
		
	} else if (*(attr->data) == '/') {			/* ls /absolute/path */
		strcpy(path, attr->rootdir);
		strcat(path, attr->data);
		
	} else {		/* /root/dir + /current/work/directory + subdir */			
		strcpy(path, attr->cwd);
...
int depth_resolve_path(struct client_attr *attr, char *path)
{
	...
	if (security_checking(attr, path) == -1)	/* Security-checking failed */
		return -1;
	
	/* If not any string in attr->cwd, the security-checking is okay, but
	 * we need add root string "/" to the attr->cwd, ensure the mechanism work
	 * correctly that all depends on it */
...
char *get_download_file(struct client_attr *attr, char *pathname)
{
	...
	if ((p = strrchr(attr->data, '/')) != NULL) {
		*p = 0;
		...
		/* 	Normally, there is no '/' at the end of attr->cwd, except attr->
		cwd is a rootdir which are "rootdir/" */
